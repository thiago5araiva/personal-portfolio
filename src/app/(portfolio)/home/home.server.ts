...
function toFeatured(items: PostDataItem[]): FeaturedItem[] {
    return items.map((i) => ({
...
function resolveFeatured(
    items: PostDataItem[],
    topSlugs: TopSlugEntry[]
): FeaturedItem[] {
    if (!items.length) return []

    const topPosts = topSlugs
        .map((entry) => items.find((i) => i.fields.slug === entry.slug))
        .filter((i): i is PostDataItem => Boolean(i))
    ...
    const fallback = items.filter(
        (i) => !usedIds.has(i.sys.id) && i.fields.tag === 'frontend'
    )
