...
const formatDate = (iso: string) => new Date(iso).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }).toUpperCase()
						...
						<article className="grid grid-cols-12 gap-x-4">
							...
							<div className="col-span-12 md:col-start-1 md:col-span-7 mt-[clamp(1.5rem,2.5vw,2.5rem)] flex items-center gap-3 font-mono text-[0.75rem] uppercase tracking-meta text-caesar-black/55">
								...
								<span
									...
									aria-hidden
								/>
								<span className="transition-colors duration-500 ease-out-quart group-hover:text-caesar-burgundy">READ →</span>
							</div>
						</article>
					</Link>
				)
